...
async function downloadBookingsCSVController(req, res) {
  try {
    const { organizer_id, event_id, booking_event_date } = req.query;
  
    // Validate mandatory parameters
    if (!organizer_id || !event_id || !booking_event_date) {
      return res.status(400).json({
        error: 'Missing required parameters: organizer_id, event_id, booking_event_date',
      });
    }
